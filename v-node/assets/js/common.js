...
$(function () {
    ...
    class Login {
        constructor () {
            ...
            this.type = 'login'
            this.reg = /^(?!([a-zA-Z]+|\d+)$)[a-zA-Z\d]{6,16}$/ // 密码校验规则
            this.formData = null
        ...
        submitForm () {
            ...
            if (this.type === 'login') {
                ...
                if (this.type === 'bind') {
                    ...
                    if (!this.reg.test($pwd.val())) {
                        $pwd.closest('.form-item').find('.error').html('密码长度6~16位，且同时包含字母和数字')
                    } else {
                        $pwd.closest('.form-item').find('.error').empty()
                        sendData = {
                            ...sendData,
                    ...
                    if (!isPoneAvailable($phone.val()) || !$imgCode.val() || !$textCode.val() || !this.reg.test($pwd.val())) {
                        hasError = true
                    } else {
                        hasError = false
                    }
