...
// new LiveConfig({'autoplay': false}).init()

/* function isQQ () {
    let ua = navigator.userAgent.toLowerCase()
    console.log(ua.match(/QQ/i))
    if (ua.match(/QQ/i) !== null && ua.match(/QQ/i).toString().toLowerCase() === 'qq') {
        new LiveConfig({'autoplay': false}).init()
    } else {
        new LiveConfig({'autoplay': true}).init()
    }
}
isQQ() */
...
$(function () {
    ...
    // let contBoxTop = $('.cont-box').offset().top
    // let windowH = $(window).height() - contBoxTop
    // $('.live-text').height(windowH)
    // 房间号
